...
const FRAME_DIR = path.join(ROOT, "assets", "gifs", "static_hero_frames");
const OUT_GIF = path.join(ROOT, "assets", "gifs", "platform_hero_overview.gif");
...
async function main() {
  ...
  const page = await browser.newPage({ viewport: { width: 1365, height: 520 }, deviceScaleFactor: 1 });
  ...
  const py = [
    ...
    "    img = img.crop((0, 0, img.width, min(img.height, 430)))",
    "    img.thumbnail((960, 330), Image.Resampling.LANCZOS)",
    ...
    "images[0].save(out, save_all=True, append_images=images[1:], duration=1050, loop=0, optimize=True)",
    "print(out)",
  ].join("\n");
  const pyPath = path.join(FRAME_DIR, "_build_static_hero_gif.py");
